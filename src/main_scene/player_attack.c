...
static void attack_knife(player_t *player)
{
    if (player->save->collision) {
        player->save->collision->life -= 50;
    }
}

static void attack_gun(game_t *game, sfRenderWindow *window)
{
...
, char *animation, int *r)
{
    ...
    *r = starset_play_animation(game->player.save, "player", animation, 3);
    ...
    if (*r == 1)
        *r = 2;
    if (compare(game->player.selected, "knife"))
        attack_knife(&game->player);
    else
        attack_gun(game, window);
    return (7);
}
...
, sfRenderWindow *window)
{
    ...
    (game->player.selected, "torch") || compare(game->player.selected, "knife"))) {
